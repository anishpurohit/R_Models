...
## function for regularized random forest
RRF <- function(X_train,y,X_test=data.frame(),cv=5,ntree=50,nodesize=5,seed=123,metric="auc",plot=0,importance=0)
{
  ...
  for (i in 1:cv)
  {
    ...
    # building model
    model_rrf <- RRF(result ~., data=X_build, ntree=ntree, nodesize=nodesize)
    
    if (plot == 1)
    {
      varImpPlot(model_rrf)
    }
    
    if (importance == 1)
    {
      print(model_rrf$importance)
    }
    
    # predicting on validation data
    pred_rrf <- predict(model_rrf, X_val, type = "prob")
    
    X_val <- cbind(X_val, pred_rrf)
    
    # predicting on test data
    if (nrow(X_test) > 0)
    {
      pred_rrf <- predict(model_rrf, X_test, type = "prob")
    }
      
    cat("CV Fold-", i, " ", metric, ": ", score(X_val$result, X_val$pred_rrf, metric), "\n", sep = "")
    ...
    if (i > 1)
    {
      ...
      if (nrow(X_test) > 0)
      {
        X_test_preds <- (X_test_preds * (i-1) + pred_rrf)/i
      }            
    }
  ...
  # appending test predictions
  X_test <- cbind(X_test, X_test_preds)
  
  # final evaluation score
  output <- output[order(output$order),]
  cat("\nRandomForest ", cv, "-Fold CV ", metric, ": ", score(output$result, output$pred_rrf, metric), "\n", sep = "")

  output <- subset(output, select = c("order", "pred_rrf"))
